...
def handler_class_for_type(type, re=re.compile('^([^<]+)(<.*>)?$')):
    ...
    if klass == 'AK::Array': # works
        return AKArray
    elif klass == 'AK::Atomic': # works
        return AKAtomic
        ...
        return AKString
    elif klass == 'AK::ByteString': # works
...
class AKVector:
    ...
    def children(self):
        ...
        if int(outline_buf) != 0:
            ...
            elements = get_field_unalloced(
                self.val,
                "m_inline_buffer_storage",
                inner_type_ptr
            )
